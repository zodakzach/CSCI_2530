...
void menuTwo(char canvas[][MAXCOLS], char backupCanvas[][MAXCOLS], bool& animate)
{
	...
	char menu[200] = "<A>nimate N / <F>ill / <L>ine / <B>ox / <N>ested Boxes / <T>ree / <M>ain Menu: ";
	...
	do
	{
		...
		case 'f':
			...
			if (fill != ESC) 
			{
				fillRecursive(canvas, fillPoint.row, fillPoint.col, canvas[fillPoint.row][fillPoint.col], fill, animate);
			}
			break;
		case 'B':
		case 'b':
			...
			{
				drawBox(canvas, boxCenter, size, animate);
			}
		...
		case 'l':
			...
			if (startPoint != ESC) 
			{
				...
				if (endPoint != ESC) 
				{
					drawLine(canvas, startLine, endLine, animate);
				}
			}
			break;
		case 'n':
		case 'N':
			...
			{
				drawBoxesRecursive(canvas, NestedBoxPoint, largestBox, animate);
			}
		...
		case 't':
			...
			if (tree != ESC && tree != 'C' && tree != 'c')
			{
				...
				treeRecursive(canvas, TreeDraw, treeHeight, 270, branchAngle, animate);
			}
			if (tree == 'c' || tree == 'C')
			{
